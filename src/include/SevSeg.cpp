...
void SevSeg::DisplayString(const char* toDisplay, byte DecAposColon)
{
	...
	for(byte digit = 1 ; digit < (numberOfDigits+1) ; digit++)
	{
		...
		if (digit == DecAposColon) /*((DecAposColon & (1<<(digit-1))) && (digit < 9))*/ //Test DecAposColon to see if we need to turn on a decimal point
...
void SevSeg::displayVar(int var, int deci) {
  char buffer[5];
  sprintf(buffer, "%4d", var);
  //disp = String(buffer); digs = deci;
  DisplayString(buffer, deci);
}

void SevSeg::display(String str, int deci) {
  //disp = str; digs = deci;
  //int strl = str.length();
  //if(strl < 4) {
